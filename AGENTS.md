...
- `src/lib/supabase.ts` **throws at import time** if `VITE_SUPABASE_URL` or `VITE_SUPABASE_ANON_KEY` are missing, so a `.env` (git-ignored) with both is mandatory before the app renders anything.
...
1. **Against the hosted project (production parity, preferred for real data):** set `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` for project `keyldymctpsvdjllliio` and use a real login. These are secrets not present in the VM by default.
2. **Fully local, self-contained:** requires Docker + the `supabase` CLI (neither is preinstalled in the base image). Run `supabase start`, then hand-create the app tables (repo migrations won't), seed a user via the GoTrue admin API, and insert a `public.user_roles` row for it. Point `.env` at the local API (`http://127.0.0.1:54321`) and use the local publishable/anon key from `supabase status`. Newer CLIs emit `sb_publishable_...` keys — these work as the anon key with `@supabase/supabase-js`.
