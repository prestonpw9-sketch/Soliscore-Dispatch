...
export default function EstimatorPanel({ open, onClose, children }: EstimatorPanelProps) {
  ...
  // ADDED: The hook goes right here at the top!
  useEffect(() => {
    if (open) {
    ...
    // Cleanup function in case the component unmounts entirely
    return () => { document.body.style.overflow = 'unset'; };
  }, [open]);

  // It MUST go before this early return
  if (!open) return null;
      ...
      <div className="w-full max-w-5xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
          <section className="p-4 border border-slate-200 rounded-xl bg-slate-50 shrink-0">
            <h3 className="text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">Import Bid Sheets</h3>
            <ExcelImporter
              apiBase={import.meta.env.VITE_API_BASE || ""}
              onImportComplete={(summary: { imported: number }) => {
