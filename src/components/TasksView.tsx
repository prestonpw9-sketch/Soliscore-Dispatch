    ...
    <div className="space-y-6 w-full">
      ...
      <div className="bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-2 overflow-x-auto custom-scrollbar">
        ...
        {weekDates.map(d => {
          ...
          return (
            ...
            >
              <span className={isSelected ? 'opacity-80 text-xs' : 'text-slate-400 text-xs'}>{dayName}</span>
              <span>{dayNum}</span>
            </button>
          );
        })}
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
        {technicians.map(t => {
          ...
          return (
            <div key={t.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={`font-bold text-sm truncate transition-all ${isDone ? 'line-through text-slate-500' : 'text-slate-900 dark:text-white'}`}>
                          {job.customerName}
                        </div>
                        <div className="flex items-center gap-2 mt-1.5">
                          ...
                          <span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-md border ${typeColors[job.type] || 'bg-slate-100 text-slate-600'}`}>
                            {job.type}
                          </span>
                        </div>
                      </div>
        ...
        {dailyJobs.length === 0 && (
          <div className="col-span-full py-16 flex flex-col items-center justify-center text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">No Routes Active</h3>
            <p className="text-slate-500 max-w-sm mt-1">There are no field assignments scheduled for {new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}.</p>
          </div>
        )}
      </div>
    </div>
