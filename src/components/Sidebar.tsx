...
interface SidebarProps {
  activeView: 'dashboard' | 'calendar' | 'schedule' | 'customers' | 'settings' | string;
  onChange: (view: string) => void;
  open: boolean;
  onClose: () => void;
}

export default function Sidebar({ activeView, onChange, open, onClose }: SidebarProps) {
  ...
  const handleNavClick = (viewTarget: string) => {
    onChange(viewTarget);
    onClose(); // Automatically close mobile drawer container on navigation
  };
  ...
  return (
    <>
      ...
      <div className="flex h-screen shrink-0">
        ...
        <div className={clsx(
          ...
          <div className="h-16 flex items-center justify-between px-6 border-b border-slate-800 shrink-0">
            <div className="flex items-center">
              <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center mr-3 text-white font-black shadow-inner">S</div>
              <span className="text-white text-base font-black tracking-wider">SOLIDCORE</span>
            </div>
            
            {/* Mobile close control trigger */}
            <button 
              type="button"
              onClick={onClose}
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <button 
              ...
              className={clsx(
                ...
                activeView === 'dashboard' ? "bg-blue-600 text-white" : "hover:bg-slate-800/80 hover:text-white text-slate-400"
              )}
            >
            ...
            <button 
              ...
              className={clsx(
                "flex items-center px-3 py-2.5 rounded-xl text-sm transition-colors w-full font-bold group",
                activeView === 'calendar' ? "bg-blue-600 text-white" : "hover:bg-slate-800/80 hover:text-white text-slate-400"
              )}
            >
            ...
            <button 
              ...
              className={clsx(
                "flex items-center px-3 py-2.5 rounded-xl text-sm transition-colors w-full font-bold group",
                activeView === 'schedule' ? "bg-blue-600 text-white" : "hover:bg-slate-800/80 hover:text-white text-slate-400"
              )}
            >
            ...
            <button 
              ...
              className={clsx(
                "flex items-center px-3 py-2.5 rounded-xl text-sm transition-colors w-full font-bold group",
                activeView === 'customers' ? "bg-blue-600 text-white" : "hover:bg-slate-800/80 hover:text-white text-slate-400"
              )}
            >
          ...
          <div className="p-3 border-t border-slate-800 bg-slate-950/20 space-y-1 shrink-0">
            ...
            <button 
              ...
              className={clsx(
                ...
                activeView === 'settings' ? "bg-blue-600 text-white" : "hover:bg-slate-800/80 hover:text-white text-slate-400"
              )}
            >
        ...
        {/* Inline Drawer slide-out panels placement logic */}
