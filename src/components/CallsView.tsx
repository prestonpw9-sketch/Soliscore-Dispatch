...
import type { Call, Priority } from '@/lib/data';
...
const CallsView: React.FC<Props> = ({
  calls,
  onSchedule,
  onComplete,
  onCallback,
            ...
            <div className="flex items-start gap-3 flex-wrap sm:flex-nowrap">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h4 className="font-semibold text-slate-900 truncate dark:text-slate-100">
                    {call.customerName}
                  </h4>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full border capitalize ${
                      priorityStyles[call.priority]
                    }`}
                  >
                  ...
                  <span className="text-xs text-slate-500 flex items-center gap-1">
                    ...
                    {call.time}
                    {call.duration && (
                      <span className="ml-1">• {call.duration}</span>
                    )}
                  </span>
                </div>
                ...
                {call.status !== 'completed' ? (
                  <>
                    <button
                      ...
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white text-xs font-medium rounded-lg transition-colors"
                    >
                    ...
                    {(call.status === 'missed' || call.status === 'callback') && (
                      <button
                        ...
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-medium rounded-lg transition-colors"
                      >
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium rounded-lg transition-colors dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
                    >
                  ...
                  <span className="text-xs text-emerald-600 font-medium flex items-center gap-1 mt-2 sm:mt-0">
                    <CheckCircle2 className="w-4 h-4" />
                    Resolved
                  </span>
                )}
