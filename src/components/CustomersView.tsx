...
const MOCK_BUILDER_PROJECTS: BuilderProject[] = [
            ...
            <div className="p-5 space-y-4 text-sm overflow-y-auto flex-1">
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 space-y-3 border border-slate-100 dark:border-slate-700/50">
                <div className="flex items-start gap-3">
                  ...
                  <a href={`tel:${selected.phone}`} className="text-teal-600 dark:text-teal-400 hover:underline font-medium">
                    {selected.phone}
                  </a>
                </div>
                <div className="flex items-start gap-3">
                  <Mail className="w-4 h-4 text-slate-400 mt-0.5" aria-hidden="true" />
                  <a href={`mailto:${selected.email}`} className="text-teal-600 dark:text-teal-400 hover:underline break-all">
                    {selected.email}
                  </a>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="pt-2">
                  ...
                  <div className="space-y-2">
                    {MOCK_BUILDER_PROJECTS.map(proj => (
                      <div
                        key={proj.id}
                        className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 hover:border-purple-400 dark:hover:border-purple-500 transition-colors bg-white dark:bg-slate-800"
                      >
                        <div className="flex justify-between items-start mb-1">
                          <span className="font-semibold text-slate-900 dark:text-slate-100">{proj.name}</span>
                          <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300">
                            {proj.status}
                          </span>
                        </div>
                        <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2">
                          <FileText className="w-3 h-3" aria-hidden="true" />
                          {proj.activePhases} Active Phases · Updated {proj.lastUpdate}
                        </div>
                      </div>
                    ))}
                    <button
          ...
          >
            ...
            <div className="bg-gradient-to-r from-purple-700 to-purple-900 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 id="builder-modal-title" className="text-lg font-bold text-white">
            ...
            <form onSubmit={handleSaveBuilder} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="builder-phone" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    Phone Number
                  </label>
                  <input
                    id="builder-phone"
                ...
                <div>
                  <label htmlFor="builder-email" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    Email
                  </label>
                  <input
                    id="builder-email"
              ...
              <div>
                <label htmlFor="builder-address" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  Main Office Address
                </label>
                <input
                  id="builder-address"
              ...
              <div>
                <label htmlFor="builder-city" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  City
                </label>
                <input
                  id="builder-city"
