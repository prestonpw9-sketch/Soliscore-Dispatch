      ...
      <div className="w-1/3 border-r border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {uniquePhones.length === 0 ? (
            <div className="p-8 text-center text-slate-400 text-sm font-medium">No active field communications.</div>
          ) : (
            uniquePhones.map((phone) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 border-b border-slate-200 dark:border-slate-800/50 cursor-pointer transition-all flex gap-3 ${
                    isActive 
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${isActive ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400' : 'bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400'}`}>
                    <Smartphone className="w-5 h-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-slate-900 dark:text-white">{phone}</div>
                    <div className={`text-xs truncate mt-0.5 ${isActive ? 'text-slate-600 dark:text-slate-300' : 'text-slate-500 dark:text-slate-500'}`}>
                      {lastMsg?.direction === 'outbound' ? <span className="font-bold">You: </span> : ''}
      ...
      <div className="w-2/3 flex flex-col bg-slate-50/30 dark:bg-slate-900 relative">
        {activePhone ? (
          <>
            ...
            <div className="px-6 py-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center shadow-sm z-10 shrink-0">
              <div className="flex items-center gap-3">
                <div className="font-black text-slate-900 dark:text-white text-xl tracking-tight">{activePhone}</div>
              </div>
                  ...
                  <div key={idx} className={`flex ${isCustomer ? 'justify-start' : 'justify-end'}`}>
                    <div className="flex flex-col gap-1 max-w-[75%]">
                      <div className={`flex items-center gap-2 mb-1 ${isCustomer ? 'justify-start' : 'justify-end'}`}>
                        {!isCustomer && <Bot className="w-3.5 h-3.5 text-indigo-500" />}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white">Signal Standby</h3>
            <p className="text-slate-500 mt-1 font-medium">Select an active field route to monitor comms.</p>
          </div>
        )}
      </div>
    </div>
