...
// ==========================================
// SOLIDCORE MASTER PRICE BOOK (From Excel Sheets)
// ==========================================
const CATALOG = {
  dwv: [
    // --- PVC DWV ---
...
export default function QuickBidEstimator({
  ...
  useEffect(() => {
    if (initialBidId) loadBid(initialBidId);
  }, [initialBidId]);

  async function loadBid(id: string) {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('project_bids').select('*').eq('id', id).single();
  ...
  function addLine() {
    ...
    setLines(prev => [...prev, { id, size: '', item: '', qty: 1, unit_price: 0 }]);
  }
  ...
  function addFromCatalog(catalogItem: { size: string, item: string, price: number }) {
    ...
    if (existingIndex >= 0) {
      ...
      const id = typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : Math.random().toString(36).slice(2);
      setLines(prev => [...prev, { id, size: catalogItem.size, item: catalogItem.item, qty: 1, unit_price: catalogItem.price }]);
    }
  }

  const filteredCatalog = CATALOG[activeCatalogTab].filter(item => 
    item.item.toLowerCase().includes(searchQuery.toLowerCase()) || 
    item.size.toLowerCase().includes(searchQuery.toLowerCase())
  );

  async function handleSaveAsMaster() { /* Connect to Supabase when ready */ }
  async function handleSaveToProject() { /* Connect to Supabase when ready */ }
  async function handleUpdate(bidId?: string) { /* Connect to Supabase when ready */ }

  if (isQuoteView) {
    return (
      <div className="p-8 bg-white min-h-full">
        <div className="flex justify-between items-center mb-8 border-b pb-4 print:hidden">
          <button onClick={() => setIsQuoteView(false)} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 font-medium transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Editor
          </button>
          <button onClick={() => window.print()} className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-lg hover:bg-black transition-colors">
        ...
        <div className="max-w-4xl mx-auto text-slate-800">
          <div className="flex justify-between items-start mb-12">
            <div>
              <h1 className="text-4xl font-black tracking-tight text-slate-900 mb-2">SOLIDCORE PLUMBING</h1>
              <p className="text-slate-500 font-medium">Tucson, AZ • ROC #XXXXXX</p>
            </div>
            <div className="text-right">
              <h2 className="text-2xl font-bold text-slate-300 uppercase tracking-widest mb-2">Estimate</h2>
            ...
            <tbody>
              {lines.map((line, idx) => (
                <tr key={line.id} className={idx % 2 === 0 ? 'bg-slate-50' : ''}>
                  ...
                  <td className="py-3 px-2 border-b border-slate-100 text-right">${Number(line.unit_price).toFixed(2)}</td>
                  <td className="py-3 px-2 border-b border-slate-100 text-right font-semibold">${(line.qty * line.unit_price).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-end">
            <div className="w-1/3 border-t-2 border-slate-800 pt-4 flex justify-between items-center">
              ...
              <span className="text-2xl font-black text-slate-900">${calcTotal().toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex h-full bg-white relative">
      {/* LEFT SIDE: MATERIAL CATALOG */}
      {isCatalogOpen && (
        <div className="w-[350px] border-r border-slate-200 bg-slate-50 flex flex-col h-full shrink-0">
          <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-white">
            <h3 className="font-bold flex items-center gap-2"><BookOpen className="w-4 h-4 text-blue-600"/> Price Book</h3>
            <button onClick={() => setIsCatalogOpen(false)} className="text-slate-400 hover:text-slate-700"><X className="w-4 h-4"/></button>
          </div>
          
          <div className="flex border-b border-slate-200 bg-white text-[11px] font-bold text-slate-600 uppercase tracking-wider">
            <button onClick={() => setActiveCatalogTab('dwv')} className={`flex-1 py-3 text-center border-b-2 transition-colors ${activeCatalogTab === 'dwv' ? 'border-blue-600 text-blue-700 bg-blue-50/50' : 'border-transparent hover:bg-slate-50'}`}>DWV / CI</button>
            <button onClick={() => setActiveCatalogTab('copper')} className={`flex-1 py-3 text-center border-b-2 transition-colors ${activeCatalogTab === 'copper' ? 'border-blue-600 text-blue-700 bg-blue-50/50' : 'border-transparent hover:bg-slate-50'}`}>Cop / Gas</button>
            <button onClick={() => setActiveCatalogTab('fixtures')} className={`flex-1 py-3 text-center border-b-2 transition-colors ${activeCatalogTab === 'fixtures' ? 'border-blue-600 text-blue-700 bg-blue-50/50' : 'border-transparent hover:bg-slate-50'}`}>Fixtures</button>
            <button onClick={() => setActiveCatalogTab('labor')} className={`flex-1 py-3 text-center border-b-2 transition-colors ${activeCatalogTab === 'labor' ? 'border-blue-600 text-blue-700 bg-blue-50/50' : 'border-transparent hover:bg-slate-50'}`}>Labor</button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {filteredCatalog.map(item => (
              <div key={item.id} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:border-blue-300 transition-colors group flex items-center justify-between cursor-pointer" onClick={() => addFromCatalog(item)}>
                <div>
                  <div className="font-bold text-slate-800 text-sm leading-tight">{item.item}</div>
                  <div className="text-xs text-slate-500 mt-1">{item.size} • <span className="font-semibold text-emerald-600">${item.price.toFixed(2)}</span></div>
                </div>
            ...
            {filteredCatalog.length === 0 && <div className="text-center p-4 text-sm text-slate-500">No items found.</div>}
          </div>
        </div>
      )}

      {/* RIGHT SIDE: THE ESTIMATE BUILDER */}
      <div className="flex-1 flex flex-col h-full p-4 overflow-hidden">
        <div className="flex gap-4 mb-6">
          <input value={projectName} onChange={e => setProjectName(e.target.value)} placeholder="Project name" className="border border-slate-300 rounded-lg p-2.5 flex-1 focus:ring-2 focus:ring-blue-500 outline-none transition-shadow font-semibold text-lg" />
          <input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Builder / Client" className="border border-slate-300 rounded-lg p-2.5 w-72 focus:ring-2 focus:ring-blue-500 outline-none transition-shadow text-lg" />
        </div>
        ...
        <div className="space-y-3 flex-1 overflow-y-auto pr-2 pb-4">
          ...
          {lines.map(line => (
            <div key={line.id} className="grid grid-cols-12 gap-2 items-center bg-slate-50 p-2 rounded-lg border border-slate-200 group hover:border-blue-300 transition-colors">
              <input className="col-span-2 border border-slate-200 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white" value={line.size} onChange={e => updateLine(line.id, { size: e.target.value })} placeholder="Size" />
              <input className="col-span-5 border border-slate-200 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white font-medium" value={line.item} onChange={e => updateLine(line.id, { item: e.target.value })} placeholder="Description" />
              <input type="number" className="col-span-1 border border-slate-200 rounded p-2 text-center focus:ring-2 focus:ring-blue-500 outline-none bg-white font-bold" value={line.qty} onChange={e => updateLine(line.id, { qty: Number(e.target.value) })} min={0} />
              <div className="col-span-2 relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-medium">$</span>
                <input type="number" className="w-full border border-slate-200 rounded py-2 pl-7 pr-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white" value={line.unit_price} onChange={e => updateLine(line.id, { unit_price: Number(e.target.value) })} min={0} step="0.01" />
              </div>
              <div className="col-span-1 text-right font-bold text-slate-700">${(line.qty * line.unit_price).toFixed(2)}</div>
              <div className="col-span-1 flex justify-center">
                <button onClick={() => removeLine(line.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors opacity-0 group-hover:opacity-100" title="Remove"><Trash2 className="w-5 h-5" /></button>
              </div>
            </div>
          ))}
          ...
          <div className="flex gap-2 mt-4 pt-2">
            {!isCatalogOpen && (
              <button onClick={() => setIsCatalogOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-lg transition-colors border border-blue-200">
                <BookOpen className="w-4 h-4" /> Open Price Book
              </button>
            )}
            <button onClick={addLine} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-lg transition-colors border border-slate-300">
              + Custom Line Item
            </button>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-200 flex flex-wrap gap-3 items-center justify-between bg-white shrink-0">
          <div className="flex gap-2">
            <button onClick={handleSaveToProject} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors shadow-sm" disabled={loading}>Save Estimate</button>
          </div>
          <div className="flex gap-4 items-center">
            ...
            <button onClick={() => setIsQuoteView(true)} className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg shadow-sm transition-all">
