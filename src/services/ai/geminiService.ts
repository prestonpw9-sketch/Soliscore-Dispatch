...
export class GeminiService implements IAIProvider {
  ...
  private buildSystemPrompt(ctx: SOLIDCOREContext): string {
    ...
    if (ctx.selectedJob)                     lines.push(`Focused job: #${ctx.selectedJob.id} — ${ctx.selectedJob.customerName}, Phase: ${ctx.selectedJob.phase}, Tech: ${ctx.selectedJob.tech}.`);
    return lines.join('\n');
  }
}